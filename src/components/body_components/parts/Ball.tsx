...
export default function Ball({ ...props }) {
  ...
  const pointClick = (event: any) => {
    if (typeof window.triggerCategoryClick === "function") {
      ...
      window.triggerCategoryClick(
        category.id,
        category.name !== activePart ? true : false
      );
    }
    //setActive(!active)
    ...
    <group {...props}>
      ...
      >
        ...
        <meshStandardMaterial
          color={hovered || activePart === category.name ? "orange" : "blue"}
        />
      </mesh>
      {(activePart === category.name || hovered) && category && (
        <>
          {activePart === category.name &&
            category.children &&
          ...
          <Html scale={0.3} position={[0, 0.2, 0.1]} transform occlude>
            <div className="annotation">
              <h2 style={{ margin: "0px", fontSize: "1.6em" }}>
                {category.name}
              </h2>
      ...
      {category.name === "Cardio" && (
        <Suspense>
          <Image
            scale={0.3}
            position={[0, 0, activePart === category.name ? 0.1 : -0.2]}
            url="/body-parts/heart.svg"
          />
        </Suspense>
      )}
    </group>
